    ...
    for trip_id in df_trips["trip_id"]:
        ...
        for i in range(len(stop_times)):
            ...
            if (len(stop_times["stop_id"]) == i + 1):
                ...
                bus_stop_csv["city_area_id"].append("")
                bus_stop_csv["stop_name"].append(f"{stop_line['stop_name'].iloc[0]}")
                bus_stop_csv["stop_lat"].append(f"{stop_line['stop_lat'].iloc[0]}")
                bus_stop_csv["stop_lon"].append(f"{stop_line['stop_lon'].iloc[0]}")
            ...
            for j in list_name_location:
                if (cv.pointPolygonTest(j[1], scaled_point, False) >= 0):
                    ...
                    bus_stop_csv["arrival_time"].append(f"{stop_times_line['arrival_time'].iloc[i]}")
                    bus_stop_csv["departure_time"].append(f"{stop_times_line['departure_time'].iloc[i]}")
                    bus_stop_csv["city_area_id"].append(f"{j[0]}")
                    bus_stop_csv["stop_name"].append(f"{stop_line['stop_name'].iloc[0]}")
                    bus_stop_csv["stop_lat"].append(f"{stop_line['stop_lat'].iloc[0]}")
                    bus_stop_csv["stop_lon"].append(f"{stop_line['stop_lon'].iloc[0]}")
        ...
        if(should_add):
            for i in range(len(bus_stop_csv["stop_id"])):
                ...
                bus_stop_csv_true["city_area_id"].append(bus_stop_csv["city_area_id"][i])
